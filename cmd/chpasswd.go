...
func shouldCheckChpasswdDatabaseFile(databaseType string) bool {
...
var ChpasswdCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if shouldCheckChpasswdDatabaseFile(flags.DatabaseType) {
			databaseFile := flags.DatabaseFile
			if databaseFile == "" {
			...
			if _, err := os.Stat(databaseFile); os.IsNotExist(err) {
				...
				cmd.Println("Unable to access database file:", err)
				return
			}
		}
		user := &models.User{}
