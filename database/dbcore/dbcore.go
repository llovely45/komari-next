...
func backupOnVersionUpgrade(databaseType string) {
	if versionID == "" || databaseType != flags.DatabaseTypeSQLite {
		return
	}
...
func databaseDSN() string {
	return strings.TrimSpace(flags.DatabaseDSN)
}

func databaseDialector(databaseType, value string) (gorm.Dialector, error) {
	switch flags.NormalizeDatabaseType(databaseType) {
	case flags.DatabaseTypeSQLite:
		return sqlite.Open(buildSQLiteDSN(value)), nil
	case flags.DatabaseTypePostgres:
		dsn := strings.TrimSpace(value)
		if dsn == "" {
			return nil, fmt.Errorf("postgres database DSN is empty")
		}
		return postgres.Open(dsn), nil
	default:
		return nil, fmt.Errorf("unsupported database type: %s (supported: %s)", databaseType, flags.SupportedDatabaseTypes())
	}
}
...
func doInitialize() error {
	var err error
	databaseType := flags.ApplyDatabaseTypeNormalization()
	dsn := databaseDSN()
	// databaseType and dsn are startup snapshots. Public flag mutation after
	// this point does not change this initialization boundary.

	// 在数据库初始化前执行：如果存在 ./data/backup.zip，则进行恢复逻辑
	if databaseType == flags.DatabaseTypeSQLite {
		func() {
			backupZipPath := filepath.Join(".", "data", "backup.zip")
	...
	case flags.DatabaseTypePostgres:
		dialector, dialectorErr := databaseDialector(databaseType, dsn)
		if dialectorErr != nil {
			return dialectorErr
		}
		instance, err = gorm.Open(dialector, logConfig)
		if err != nil {
			return fmt.Errorf("failed to connect to PostgreSQL: %w", err)
		}
		sqlDB, dbErr := instance.DB()
		if dbErr != nil {
			return fmt.Errorf("open PostgreSQL connection pool: %w", dbErr)
		}
		...
		sqlDB.SetConnMaxIdleTime(0)
	default:
		return fmt.Errorf("unsupported database type: %s (supported: %s)", databaseType, flags.SupportedDatabaseTypes())
	}
	...
	backupOnVersionUpgrade(databaseType)
