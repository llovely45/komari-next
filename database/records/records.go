...
// 历史监控数据已完全迁移到共享 PostgreSQL 上的 metric store，并通过 metric_
// 表名前缀与控制面表分开。旧的 records / records_long_term / gpu_records /
// ping_records 表仅在启动一次性迁移时作为数据源读取，运行期的读写全部走 metric store。
//
