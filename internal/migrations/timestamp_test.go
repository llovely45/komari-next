...
func TestShouldMigrateLegacyTimestampOnlyForSQLite(t *testing.T) {
	for _, dialect := range []string{"sqlite", "sqlite3"} {
		if !shouldMigrateLegacyTimestamp(dialect) {
			t.Fatalf("%s should run legacy timestamp migration", dialect)
		}
	}
	for _, dialect := range []string{"postgres", "mysql"} {
		if shouldMigrateLegacyTimestamp(dialect) {
			t.Fatalf("%s should skip SQLite-only timestamp migration", dialect)
		}
	}
}
