...
func (m *Module) resolveNodePathAt(name, cwd string, allowMissing bool, mode nodePathMode) (string, error) {
	...
	if mode == nodePathNoFollowFinal {
		parent, err := filepathutil.ResolvePath(filepath.Dir(resolved), allowMissing)
		if err != nil {
			return "", err
		}
		resolved = filepath.Clean(filepath.Join(parent, filepath.Base(resolved)))
	...
	resolved, err := filepathutil.ResolvePath(resolved, allowMissing)
	if err != nil {
		return "", err
	}
	if !m.withinAnyRoot(resolved) {
