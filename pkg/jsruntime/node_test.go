...
func TestHTTPClientRequestErrorWithoutListenerStillCloses(t *testing.T) {
	...
	time.Sleep(80 * time.Millisecond)
	// The request error is reported by an asynchronous event-loop callback.
	// Stop the loop before reading the buffer so the test does not race that
	// callback's console write.
	runtime.Close()
