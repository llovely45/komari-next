...
func TestConfinedSourceLoaderAcceptsEquivalentAbsolutePath(t *testing.T) {
	baseDir := t.TempDir()
	modulePath := filepath.Join(baseDir, "module.js")
	if err := os.WriteFile(modulePath, []byte(`module.exports = "ok";`), 0o600); err != nil {
		t.Fatal(err)
	}
	canonicalBaseDir, err := resolveBaseDir(baseDir)
	if err != nil {
		t.Fatal(err)
	}

	loader := confinedSourceLoader(canonicalBaseDir, require.DefaultSourceLoader)
	if _, err := loader(modulePath); err != nil {
		t.Fatalf("confined loader rejected equivalent absolute path %q under %q: %v", modulePath, canonicalBaseDir, err)
	}
}
