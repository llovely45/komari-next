// Package pluginprocess contains the versioned wire model and SDK shared by
// Komari's Go/WASI plugin host and Go plugin modules. It intentionally has no
// database dependency and exposes host operations only through the RPC API.
...
// Capability is a host capability requested by a Go/WASI plugin.
...
// Handshake is the first message exchanged when a Go/WASI plugin starts.
...
// HandshakeResponse acknowledges the protocol and capability negotiation.
// An explicit response envelope lets the host reject startup without leaving
// the guest guessing why its stream closed.
...
func isKnownCapability(capability Capability) bool {
	switch capability {
	case CapabilityEvents, CapabilityRPC, CapabilityRoutes, CapabilityMetrics, CapabilityExec, CapabilityNetwork:
		return true
	default:
		return false
	}
...
// Message is the stable JSON Lines envelope. Payload is application-defined
// JSON encoded as bytes (and therefore base64-encoded inside this envelope).
type Message struct {
