...
func supportsDatabaseDownloadBackup(databaseType string) bool {
	return flags.NormalizeDatabaseType(databaseType) == flags.DatabaseTypeSQLite
}
...
func DownloadBackup(c *gin.Context) {
	if !supportsDatabaseDownloadBackup(flags.DatabaseType) {
		api.RespondError(c, http.StatusNotImplemented, "PostgreSQL database backup is not supported by this endpoint; use pg_dump for the database and archive data separately")
		return
	}
